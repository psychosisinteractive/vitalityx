...
unsigned char ide_print_error(unsigned int drive, unsigned char err) {
    ...
    tty_pputstring("IDE:");
    if (err == 1) {tty_pputstring("- Device Fault\n     "); err = 19;}
    else if (err == 2) {
        unsigned char st = ide_read(ide_devices[drive].Channel, ATA_REG_ERROR);
        if (st & ATA_ER_AMNF)   {tty_pputstring("- No Address Mark Found\n     ");   err = 7;}
        if (st & ATA_ER_TK0NF)   {tty_pputstring("- No Media or Media Error\n     ");   err = 3;}
        if (st & ATA_ER_ABRT)   {tty_pputstring("- Command Aborted\n     ");      err = 20;}
        if (st & ATA_ER_MCR)   {tty_pputstring("- No Media or Media Error\n     ");   err = 3;}
        if (st & ATA_ER_IDNF)   {tty_pputstring("- ID mark not Found\n     ");      err = 21;}
        if (st & ATA_ER_MC)   {tty_pputstring("- No Media or Media Error\n     ");   err = 3;}
        if (st & ATA_ER_UNC)   {tty_pputstring("- Uncorrectable Data Error\n     ");   err = 22;}
        if (st & ATA_ER_BBK)   {tty_pputstring("- Bad Sectors\n     ");       err = 13;}
    } else  if (err == 3)           {tty_pputstring("- Reads Nothing\n     "); err = 23;}
        else  if (err == 4)  {tty_pputstring("- Write Protected\n     "); err = 8;}
